...
CREATE TABLE repository_activity_collection (
  ...
  source_key text NOT NULL CHECK(source_key<>''),window_started_at timestamptz NOT NULL,window_ended_at timestamptz NOT NULL,
  commits_status text NOT NULL CHECK(commits_status IN ('AVAILABLE','PARTIAL','NOT_COLLECTED','PERMISSION_REQUIRED','ERROR')),
  pull_requests_status text NOT NULL CHECK(pull_requests_status IN ('AVAILABLE','PARTIAL','NOT_COLLECTED','PERMISSION_REQUIRED','ERROR')),
  limitations jsonb NOT NULL DEFAULT '[]' CHECK(jsonb_typeof(limitations)='array'),collected_at timestamptz NOT NULL DEFAULT now(),
  CHECK(window_ended_at>=window_started_at),UNIQUE(tenant_id,repository_entity_id,source_key)
);
CREATE TABLE repository_activity_event (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES tenant(id),repository_entity_id uuid NOT NULL REFERENCES entity(id) ON DELETE CASCADE,
  provider text NOT NULL CHECK(provider IN ('GITHUB')),provider_event_key text NOT NULL CHECK(provider_event_key<>''),
  event_type text NOT NULL CHECK(event_type IN ('COMMIT','PULL_REQUEST_OPENED','PULL_REQUEST_MERGED')),
  occurred_at timestamptz NOT NULL,title text NOT NULL CHECK(title<>''),actor_key text,actor_login text,actor_avatar_url text,actor_is_bot boolean NOT NULL DEFAULT false,
  revision text,branch text,pull_request_number integer CHECK(pull_request_number IS NULL OR pull_request_number>0),source_url text,
  metadata jsonb NOT NULL DEFAULT '{}' CHECK(jsonb_typeof(metadata)='object'),observed_at timestamptz NOT NULL DEFAULT now(),
  CHECK((actor_key IS NULL)=(actor_login IS NULL)),
  CHECK(actor_avatar_url IS NULL OR actor_avatar_url ~ '^https://'),CHECK(source_url IS NULL OR source_url ~ '^https://'),
...
CREATE TABLE github_installation_setup(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES tenant(id),actor_key text NOT NULL CHECK(actor_key<>''),state_hash text NOT NULL UNIQUE CHECK(state_hash ~ '^sha256:[a-f0-9]{64}$'),return_to text NOT NULL CHECK(return_to ~ '^/[^/].*|^/$'),expires_at timestamptz NOT NULL,consumed_at timestamptz,installation_id text CHECK(installation_id IS NULL OR installation_id ~ '^[1-9][0-9]{0,19}$'),created_at timestamptz NOT NULL DEFAULT now(),CHECK(expires_at>created_at));
...
-- Tenant code-policy overlays and repository evaluations (migration 020).
CREATE TABLE tenant_code_function(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES tenant(id),function_key text NOT NULL CHECK(function_key ~ '^[a-z][a-z0-9.-]{1,127}$'),name text NOT NULL CHECK(name<>''),description text NOT NULL DEFAULT '',domain_key text NOT NULL CHECK(domain_key ~ '^[a-z][a-z0-9.-]{1,127}$'),status text NOT NULL DEFAULT 'ACTIVE' CHECK(status IN ('ACTIVE','RETIRED')),created_by text NOT NULL,updated_by text NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),UNIQUE(tenant_id,function_key));
CREATE TABLE tenant_code_policy(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES tenant(id),function_key text NOT NULL CHECK(function_key ~ '^[a-z][a-z0-9.-]{1,127}$'),function_source text NOT NULL CHECK(function_source IN ('PRIMARY','CUSTOM')),allowed_technology_ids uuid[] NOT NULL DEFAULT '{}',prohibited_technology_ids uuid[] NOT NULL DEFAULT '{}',policy_fingerprint text NOT NULL CHECK(policy_fingerprint ~ '^sha256:[a-f0-9]{64}$'),updated_by text NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),UNIQUE(tenant_id,function_key),CHECK(NOT allowed_technology_ids && prohibited_technology_ids));
CREATE TABLE repository_code_policy_evaluation(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES tenant(id),repository_entity_id uuid NOT NULL REFERENCES entity(id) ON DELETE CASCADE,policy_set_fingerprint text NOT NULL CHECK(policy_set_fingerprint ~ '^sha256:[a-f0-9]{64}$'),evidence_fingerprint text NOT NULL CHECK(evidence_fingerprint ~ '^sha256:[a-f0-9]{64}$'),status text NOT NULL CHECK(status IN ('COMPLIANT','MISALIGNED','UNASSESSED')),violation_count integer NOT NULL DEFAULT 0 CHECK(violation_count>=0),unclassified_count integer NOT NULL DEFAULT 0 CHECK(unclassified_count>=0),details jsonb NOT NULL DEFAULT '{}' CHECK(jsonb_typeof(details)='object'),evaluated_by text NOT NULL,evaluated_at timestamptz NOT NULL DEFAULT now(),UNIQUE(tenant_id,repository_entity_id,policy_set_fingerprint,evidence_fingerprint));

-- Tenant architecture policy overlays (migration 025). The canonical taxonomy and
-- reference-model artifacts ship with the API; only governed overlays are persisted.
CREATE TABLE tenant_architecture_profile(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES tenant(id),profile_key text NOT NULL CHECK(profile_key ~ '^[a-z][a-z0-9_.-]{2,127}$'),name text NOT NULL CHECK(name<>''),reference_model_key text NOT NULL CHECK(reference_model_key<>''),reference_model_version text NOT NULL CHECK(reference_model_version<>''),version integer NOT NULL DEFAULT 1 CHECK(version>0),status text NOT NULL DEFAULT 'DRAFT' CHECK(status IN ('DRAFT','ACTIVE','ARCHIVED')),state jsonb NOT NULL CHECK(jsonb_typeof(state)='object'),fingerprint text NOT NULL CHECK(fingerprint ~ '^sha256:[a-f0-9]{64}$'),created_by text NOT NULL,updated_by text NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),UNIQUE(tenant_id,profile_key));
CREATE TABLE tenant_architecture_profile_revision(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES tenant(id),profile_id uuid NOT NULL REFERENCES tenant_architecture_profile(id) ON DELETE CASCADE,version integer NOT NULL CHECK(version>0),status text NOT NULL CHECK(status IN ('DRAFT','ACTIVE','ARCHIVED')),state jsonb NOT NULL CHECK(jsonb_typeof(state)='object'),fingerprint text NOT NULL CHECK(fingerprint ~ '^sha256:[a-f0-9]{64}$'),actor_key text NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(profile_id,version));

-- Tenant-governed deterministic insight rules (migration 021). Findings are derived from
-- current graph facts at read time; these tables only govern behavior and revision history.
CREATE TABLE deterministic_insight_rule_policy(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES tenant(id),rule_key text NOT NULL CHECK(rule_key ~ '^[a-z][a-z0-9_.-]{2,127}$'),enabled boolean NOT NULL DEFAULT true,severity text NOT NULL CHECK(severity IN ('CRITICAL','HIGH','MEDIUM','LOW','INFO')),minimum_repositories integer NOT NULL DEFAULT 1 CHECK(minimum_repositories>0),configuration jsonb NOT NULL DEFAULT '{}' CHECK(jsonb_typeof(configuration)='object'),version integer NOT NULL DEFAULT 1 CHECK(version>0),updated_by text NOT NULL,updated_at timestamptz NOT NULL DEFAULT now(),UNIQUE(tenant_id,rule_key));
CREATE TABLE deterministic_insight_rule_policy_revision(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES tenant(id),rule_policy_id uuid NOT NULL REFERENCES deterministic_insight_rule_policy(id) ON DELETE CASCADE,prior_version integer NOT NULL CHECK(prior_version>=0),resulting_version integer NOT NULL CHECK(resulting_version=prior_version+1),enabled boolean NOT NULL,severity text NOT NULL CHECK(severity IN ('CRITICAL','HIGH','MEDIUM','LOW','INFO')),minimum_repositories integer NOT NULL CHECK(minimum_repositories>0),configuration jsonb NOT NULL CHECK(jsonb_typeof(configuration)='object'),actor_key text NOT NULL,changed_at timestamptz NOT NULL DEFAULT now(),UNIQUE(rule_policy_id,resulting_version));
...
CREATE INDEX idx_repository_activity_event_timeline ON repository_activity_event(tenant_id,repository_entity_id,occurred_at DESC,id DESC);
CREATE INDEX idx_repository_activity_event_actor ON repository_activity_event(tenant_id,repository_entity_id,actor_key,occurred_at DESC) WHERE actor_key IS NOT NULL;
CREATE INDEX idx_repository_activity_collection_freshness ON repository_activity_collection(tenant_id,repository_entity_id,collected_at DESC);
...
CREATE INDEX idx_fact_object_predicate_current ON fact_assertion(object_entity_id,predicate) WHERE system_to IS NULL AND object_entity_id IS NOT NULL;
CREATE INDEX idx_fact_tenant_predicate_current ON fact_assertion(tenant_id,predicate,subject_entity_id) WHERE system_to IS NULL;
CREATE INDEX idx_fact_depsdev_graph_root_current ON fact_assertion((properties->>'graph_root_purl'),object_entity_id) WHERE system_to IS NULL AND predicate='DEPENDS_ON' AND properties->>'provider'='deps.dev';
...
CREATE INDEX idx_github_installation_setup_expiry ON github_installation_setup(tenant_id,expires_at) WHERE consumed_at IS NULL;
CREATE INDEX idx_tenant_code_function_active ON tenant_code_function(tenant_id,status,function_key);
CREATE INDEX idx_tenant_code_policy_function ON tenant_code_policy(tenant_id,function_key);
CREATE INDEX idx_repository_code_policy_latest ON repository_code_policy_evaluation(tenant_id,repository_entity_id,evaluated_at DESC,id DESC);
CREATE UNIQUE INDEX uq_tenant_architecture_profile_active ON tenant_architecture_profile(tenant_id,reference_model_key,reference_model_version) WHERE status='ACTIVE';
CREATE INDEX idx_tenant_architecture_profile_list ON tenant_architecture_profile(tenant_id,status,updated_at DESC,id DESC);
CREATE INDEX idx_tenant_architecture_profile_revision ON tenant_architecture_profile_revision(tenant_id,profile_id,version DESC);
CREATE INDEX idx_deterministic_rule_policy_tenant ON deterministic_insight_rule_policy(tenant_id,enabled,rule_key);
...
CREATE INDEX idx_business_map_capability_criticality ON business_map_capability(tenant_id,criticality,business_map_id);
...
CREATE INDEX idx_business_map_application_assignment_capability ON business_map_application_assignment(tenant_id,business_map_capability_id);
CREATE INDEX idx_business_map_application_assignment_application ON business_map_application_assignment(tenant_id,application_entity_id);
...
INSERT INTO schema_migration(version,checksum) VALUES
 ...
 ('019_hosted_github_and_derived_calibration.sql','d04e2d39bb4a184ce68d9f1e8dd3151c57a37f1b47d8f88ecd26e76e56b66bba'), -- gitleaks:allow; migration checksum, not a credential
