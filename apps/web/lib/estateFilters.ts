...
export const LENSES: Record<string, { label: string; apply: Partial<EstateQuery> }> = {
  ...
  cto: { label: "CTO overview", apply: { domain: "ALL", confidence: "ALL", freshness: "ALL", sort: "priority", dir: "desc" } },
  ea: { label: "EA standardization", apply: { domain: "TECHNOLOGY", confidence: "ALL", freshness: "ALL", sort: "priority", dir: "desc" } },
  risk: { label: "CISO risk", apply: { domain: "ALL", confidence: "ALL", freshness: "ALL", sort: "viability", dir: "asc" } },
  platform: { label: "Platform drift", apply: { domain: "ENTERPRISE", confidence: "ALL", freshness: "ALL", sort: "priority", dir: "desc" } },
};
...
export function applyEstateQuery(items: RankedItem[], q: EstateQuery): RankedItem[] {
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    else if (q.sort === "viability") cmp = (a.viability?.value ?? 0) - (b.viability?.value ?? 0);
