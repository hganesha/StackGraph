...
| Acquisition-to-publication orchestration | **Verified** | [GitHub control loop](../services/enterprise-discovery/stackgraph_discovery/github_control_loop.py) leases installation/repository runs, short-circuits unchanged revisions, durably stores changed snapshots, scans, publishes, and updates freshness; projection and intelligence consume publication queues continuously. Operator rescan jobs retain durable links to every child run, including shared active runs, and roll up running, success, failure, timestamps, and terminal errors | Control-loop tests, scanner-persistence integration tests, live two-repository rescan verification, and [pipeline runbook](runbooks/continuous-discovery-pipeline.md) |
